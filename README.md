...
4. **Type converts are not counted as FLOPs.**  
  Any type conversions between different precision FP types or from/to integer types are not counted as FLOPs. We might add dedicated counters for conversion operations in the future to give a summary if such operations have significant impact on the runtime (diagnostic).

5. **AVX-512_BF16 (bfloat16) operations are counted as single precision FLOPs.**  
